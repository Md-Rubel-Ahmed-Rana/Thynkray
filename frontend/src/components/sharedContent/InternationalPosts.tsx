...
import { handleFetchInternationalNews } from "@/modules/post/api";
...
const InternationalPosts = () => {
  const { data, isLoading } = useQuery({
    queryKey: ["news", ""],
    queryFn: handleFetchInternationalNews,
  });

  const news = (data || []) as InternationalPost[];
        ...
        <>
          {news.length <= 0 ? (
            <NoDataFound message="International news was not found!">
              <Box>
                <Typography>
                  We did not found any international news.
                </Typography>

                <Typography>
                  There might be a server side error occurred. Please be
                  patient. <br /> If you want to read article or participate on
                  discussion or Chat with AI, Please select one from below
                </Typography>
                <Box
                  sx={{
                    display: "flex",
