...
export class TasksService {
  ...
  async keepDbCacheMeilisearchDataConsistence() {
    ...
    const postsFromCache = ((await this.cacheService.get("posts")) ||
      []) as GetPostDto[];
    const postsFromMeilisearch = (await this.meilisearchService.getAllPosts())
      .data;
    ...
    console.log({
      ...
      data: {
        db: {
          total: postsFromDb?.length,
          ids: postsFromDb.map((post) => post?.id),
        },
        cache: {
          total: postsFromCache?.length,
          ids: postsFromCache.map((post) => post?.id),
        },
        search: {
          total: postsFromMeilisearch?.length,
          ids: postsFromMeilisearch.map((post) => post?.id),
        },
      },
    });
  ...
  private async ensureConsistency(
    ...
    if (inconsistentCachePosts.length > 0) {
      ...
      this.logger.warn(
        `Fixed Redis cache inconsistency with ${inconsistentCachePosts.length} posts`
      );
    }
    ...
    if (inconsistentSearchPosts.length > 0) {
      ...
      this.logger.warn(
        `Fixed MeiliSearch inconsistency with ${inconsistentSearchPosts.length} posts`
      );
    }
