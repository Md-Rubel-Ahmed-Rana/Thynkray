...
export class PostService {
  ...
  async findAllByAuthorId(authorId: string) {
    ...
    if (postsFromCache !== null) {
      const posts = postsFromCache.filter(
        (post: any) => post?.author?.id === authorId
      );
      if (posts?.length > 0) {
        return {
          message: "Posts retrieved from cache!",
  ...
  async update(id: string, updatePostDto: UpdatePostDto) {
    ...
    const { title, slug, tags, content, category, thumbnail, description } =
      updatePostDto;
